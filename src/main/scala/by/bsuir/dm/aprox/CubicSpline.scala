...
object CubicSpline {
  def apply(x: Double, xy: List[(Double, Double)]): Double = {
    ...
    def binarySearch(x: Double, list: List[(Double, Double, Double, Double, Double)]): (Double, Double, Double, Double, Double) = x match {
      ...
      case (x: Double) if list.last._5 < x => list.last
      case (x: Double) if list(1 + list.length / 2)._5 < x => binarySearch(x, list.drop(1 + list.length / 2))
      case (x: Double) if x < list(1 + list.length / 2)._5 => binarySearch(x, list.dropRight(1 + list.length / 2))
    }
    ...
    val splines = xy.sliding(3).scanLeft((xy.head, (0.0, 0.0)))((acc, cur) => {
      ...
      (tuple(1)._1._2, b, tuple(1)._2, d, tuple(1)._1._1)
    }).toList
