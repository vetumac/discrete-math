...
object Holetsky {
  ...
  def apply(sle: SystemOfLinearEquationsMatrix): List[Double] = {
    ...
    def firstStep(a: SquareMatrix, l: List[List[Double]]): List[List[Double]] = l.length < a.matrix.length match {
      case true => firstStep(a, fillDiogonal(a, fillNotDiogonal(a, addZeros(l))))
      case false => l
    }

    def secondStep(sle: List[List[Double]]): List[Double] = sle.size match {
    ...
    def addZeros(matrix: List[List[Double]]): List[List[Double]] = matrix.map(f => f :+ 0.0) :+ List[Double]()

    def fillNotDiogonal(a: SquareMatrix, l: List[List[Double]]): List[List[Double]] = {
      val i = l.length - 1
      val j = l.last.length
      i > j match {
        case true =>
    ...
    def fillDiogonal(a: SquareMatrix, l: List[List[Double]]): List[List[Double]] = {
      val i = l.length - 1
      l.dropRight(1) :+ (l.last :+ sqrt(a.matrix(i)(i) - List.range(0, i).foldLeft(List[Double]())((a, k) => a :+ l(i)(k) * l(i)(k)).sum))
    }

    def sqrt(double: Double): Double = double < 0 match {
